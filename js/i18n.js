...
(function () {
  ...
  const DICT = {
    ...
    'hero.sub': {
      de: 'Drei Ebenen, ein System: langfristige Vision in fünf Lebens&shy;bereichen, fokussierte 1–12-Wochen-Challenges (ARCs) und numerische Gewohnheiten im Alltag.',
    ...
    'p1.b': {
      de: 'Gewohnheiten sind keine Free-Text-Notizen. Jedie Gewohnheit hat Zielwert, Richtung (≥ oder ≤) und Einheit. „5,2 km gelaufen", nicht „Sport gemacht".',
    ...
    'faq01.a': {
      de: 'ArcUp ist eine mobile App für <strong>strukturierte persönliche Entwicklung</strong>. Sie verbindet drei Zeit-Ebenen miteinander:<ul><li><strong>Lebensbereiche</strong> – langfristig: fünf Bereiche mit Vision, Zielen und Tagebuch (<em>Wer will ich werden?</em>)</li><li><strong>ARCs</strong> – mittelfristig: zeitlich begrenzte Challenges über 1 bis 12 Wochen (<em>Was übe ich gerade gezielt?</em>)</li><li><strong>Gewohnheiten</strong> – täglich: numerisch messbare Gewohnheiten (<em>Was tue ich heute konkret?</em>)</li></ul>Unsere Tagline: <em>ArcUp – Building Better Habits.</em>',
      ...
      es: 'ArcUp es una app móvil para el <strong>desarrollo personal estructurado</strong>. Conecta tres horizontes temporales:<ul><li><strong>Áreas de vida</strong> — a largo plazo: cinco áreas con visión, objetivos y diario (<em>¿Quién quiero ser?</em>)</li><li><strong>ARCs</strong> — a medio plazo: retos limitados en el tiempo de 1 a 12 semanas (<em>¿Qué practico ahora con intención?</em>)</li><li><strong>Hábitos</strong> — a diario: hábitos numéricamente medibles (<em>¿Qué hago hoy en concreto?</em>)</li></ul>Nuestra tagline: <em>ArcUp – Building Better Habits.</em>',
      fr: "ArcUp est une app mobile pour le <strong>développement personnel structuré</strong>. Elle relie trois horizons temporels :<ul><li><strong>Domaines de vie</strong> — long terme : cinq domaines avec vision, objectifs et journal (<em>Qui je veux devenir ?</em>)</li><li><strong>ARCs</strong> — moyen terme : défis limités dans le temps de 1 à 12 semaines (<em>Que suis-je en train de pratiquer délibérément ?</em>)</li><li><strong>Habitudes</strong> — au quotidien : habitudes mesurables numériquement (<em>Que fais-je concrètement aujourd'hui ?</em>)</li></ul>Notre tagline : <em>ArcUp – Building Better Habits.</em>",
      it: "ArcUp è un'app mobile per lo <strong>sviluppo personale strutturato</strong>. Collega tre orizzonti temporali:<ul><li><strong>Aree di vita</strong> — a lungo termine: cinque aree con visione, obiettivi e diario (<em>Chi voglio diventare?</em>)</li><li><strong>ARC</strong> — a medio termine: sfide a tempo limitato di 1 a 12 settimane (<em>Cosa sto praticando deliberatamente ora?</em>)</li><li><strong>Abitudini</strong> — quotidianamente: abitudini misurabili numericamente (<em>Cosa faccio concretamente oggi?</em>)</li></ul>La nostra tagline: <em>ArcUp – Building Better Habits.</em>",
    },
    'faq02.q': { de: 'Ist ArcUp ein klassischer Gewohnheits-Tracker?', en: 'Is ArcUp a classic habit tracker?', es: '¿ArcUp es un tracker de hábitos clásico?', fr: 'ArcUp est-il un tracker d’habitudes classique ?', it: 'ArcUp è un tracker di abitudini classico?' },
    'faq02.a': {
      de: 'Nein. Ein klassischer Gewohnheits-Tracker zeichnet „heute gemacht – ja/nein" auf. ArcUp tracked <strong>numerisch</strong>: nicht „heute gelaufen", sondern „heute 5,2 km gelaufen". Jedie Gewohnheit hat eine Einheit (km, min, kg, Wiederholungen, …), einen Zielwert, eine Richtung (≥ oder ≤) und ein Intervall (täglich oder wöchentlich). Dadurch wird Fortschritt messbar statt geschätzt.',
      ...
      es: 'No. Un tracker de hábitos clásico registra «hecho hoy — sí/no». ArcUp registra de forma <strong>numérica</strong>: no «corrí hoy», sino «corrí 5,2 km hoy». Cada hábito tiene una unidad (km, min, kg, repeticiones, …), un valor objetivo, una dirección (≥ o ≤) y un intervalo (diario o semanal). Así el progreso es medible, no estimado.',
      fr: "Non. Un tracker d'habitudes classique note « fait aujourd'hui — oui/non ». ArcUp suit <strong>numériquement</strong> : pas « couru aujourd'hui », mais « couru 5,2 km aujourd'hui ». Chaque habitude a une unité (km, min, kg, répétitions, …), une valeur cible, une direction (≥ ou ≤) et un intervalle (quotidien ou hebdomadaire). Le progrès devient mesurable, pas estimé.",
      it: "No. Un tracker di abitudini classico registra «fatto oggi — sì/no». ArcUp traccia in modo <strong>numerico</strong>: non «corso oggi», ma «corsi 5,2 km oggi». Ogni abitudine ha un'unità (km, min, kg, ripetizioni, …), un valore obiettivo, una direzione (≥ o ≤) e un intervallo (giornaliero o settimanale). Il progresso diventa misurabile, non stimato.",
    },
    ...
    'faq08.q': { de: 'Was bedeutet „Above" und „Below" bei einem Gewohnheit?', en: 'What do "Above" and "Below" mean for a habit?', es: '¿Qué significan «Above» y «Below» en un hábito?', fr: 'Que signifient « Above » et « Below » pour une habitude ?', it: 'Cosa significano «Above» e «Below» per un’abitudine?' },
    'faq08.a': {
    ...
    'faq10.q': { de: 'Wie viele Gewohnheiten sollte ein ARC haben?', en: 'How many habits should an ARC have?', es: '¿Cuántos hábitos debería tener un ARC?', fr: "Combien d'habitudes un ARC devrait-il avoir ?", it: 'Quante abitudini dovrebbe avere un ARC?' },
    'faq10.a': {
      de: 'Der Sweet Spot liegt bei <strong>3–6 Gewohnheiten</strong> pro ARC. Weniger fühlt sich substanzlos an, mehr überfordert. Eine gute Mischung aus täglichen Routine-Gewohnheiten und wöchentlichen Volumen-Zielen funktioniert in der Praxis am besten.',
    ...
    'faq12.a': {
      de: 'Du tippst auf den <strong>+ Track</strong>-Button auf dem Home-Screen und öffnest dein Logbook. Dort trägst du für jedes aktive Gewohnheit den Wert des Tages ein – per Stepper oder Slider. Zusätzlich gibt es einen <strong>Timer</strong> (Stoppuhr oder Pomodoro), mit dem du Zeit-basierte Gewohnheiten live mitlaufen lassen kannst.',
    ...
    'faq14.q': { de: 'Was bedeutet die Prozentzahl bei jedem Gewohnheit?', en: 'What does the percentage on each habit mean?', es: '¿Qué significa el porcentaje en cada hábito?', fr: 'Que signifie le pourcentage sur chaque habitude ?', it: 'Cosa significa la percentuale su ogni abitudine?' },
    'faq14.a': {
    ...
    'faq15.a': {
      de: 'Ein Tag, an dem <strong>alle</strong> deine täglichen Gewohnheits-Ziele auf 100 % gekommen sind – bei wöchentlichen Gewohnheiten am letzten Tag der Woche zusätzlich die Wochenziele. Perfekte Tage zählen für deinen Streak und sind die Grundlage für viele Achievements.',
    ...
    'faq19.a': {
      de: 'Jeder ARC und jede Gewohnheit baut auf einer <strong>Wissensgrundlage</strong> auf – Büchern, Artikeln, Vorträgen oder eigens für ArcUp redaktionell aufbereiteten Beiträgen („ArcUp Originals"). In den Lerninhalten kannst du diese direkt in der App lesen, durchsuchen, filtern und als Favoriten speichern. Du siehst zu jedem Lerninhalt, welche ARCs und Gewohnheiten darauf aufbauen.',
      en: 'Every ARC and every habit is built on a <strong>knowledge foundation</strong> — books, articles, talks or pieces produced editorially for ArcUp ("ArcUp Originals"). In Learning Content you can read these items directly in the app, search, filter and save them as favourites. For every item you see which ARCs and habits build on it.',
      es: 'Cada ARC y cada hábito se apoya en una <strong>base de conocimiento</strong> — libros, artículos, charlas o piezas redactadas editorialmente para ArcUp («ArcUp Originals»). En Contenidos de aprendizaje puedes leer estos contenidos directamente en la app, buscar, filtrar y guardar como favoritos. Para cada contenido ves qué ARCs y hábitos se apoyan en él.',
      fr: "Chaque ARC et chaque habitude s'appuie sur une <strong>base de connaissance</strong> — livres, articles, conférences ou contenus rédigés éditorialement pour ArcUp (« ArcUp Originals »). Dans Contenus d'apprentissage tu lis ces contenus directement dans l'app, tu cherches, filtres et enregistres en favoris. Pour chaque contenu tu vois quels ARC et habitudes s'y appuient.",
      it: 'Ogni ARC e ogni abitudine si basa su una <strong>fondamenta di conoscenza</strong> — libri, articoli, talk o contenuti redatti editorialmente per ArcUp («ArcUp Originals»). In Contenuti di apprendimento puoi leggere questi contenuti direttamente nell’app, cercare, filtrare e salvarli nei preferiti. Per ogni contenuto vedi quali ARC e abitudini si appoggiano su di esso.',
    },
    ...
    'faq21.a': {
      de: 'Bei einigen Lerninhalten findest du im Abschnitt „Lerninhalte mit ähnlichen Themen" Empfehlungs-Links zu Büchern bei Amazon. Diese sind mit einem <strong>Sternchen (*)</strong> gekennzeichnet und transparent als Werbung ausgewiesen: <em>„Als Amazon-Partner verdient ArcUp an qualifizierten Käufen."</em> Du zahlst dadurch keinen Cent mehr. Affiliate-Vergütungen beeinflussen unsere Empfehlungen nicht.',
      en: 'On some pieces of learning content you’ll find recommendation links to books on Amazon in the section "Related learning content". They are marked with an <strong>asterisk (*)</strong> and transparently labelled as advertising: <em>"As an Amazon Associate, ArcUp earns from qualifying purchases."</em> You don’t pay a cent more. Affiliate compensation does not influence our recommendations.',
      es: 'En algunos contenidos de aprendizaje encuentras enlaces de recomendación a libros en Amazon, en la sección «Contenidos de aprendizaje relacionados». Están marcados con un <strong>asterisco (*)</strong> y señalados de forma transparente como publicidad: <em>«Como afiliado de Amazon, ArcUp obtiene ingresos por compras cualificadas.»</em> No pagas ni un céntimo más. La comisión no influye en nuestras recomendaciones.',
      fr: "Sur certains contenus d'apprentissage tu trouves des liens de recommandation vers des livres sur Amazon, dans la section « Contenus d'apprentissage similaires ». Ils sont marqués d'un <strong>astérisque (*)</strong> et signalés de manière transparente comme publicité : <em>« En tant que partenaire Amazon, ArcUp est rémunéré pour les achats remplissant les conditions requises. »</em> Tu ne paies pas un centime de plus. La rémunération n'influence pas nos recommandations.",
      it: 'Su alcuni contenuti di apprendimento trovi link di consiglio a libri su Amazon, nella sezione «Contenuti di apprendimento correlati». Sono marcati con un <strong>asterisco (*)</strong> e segnalati in modo trasparente come pubblicità: <em>«In qualità di affiliato Amazon, ArcUp riceve un guadagno dagli acquisti idonei.»</em> Non paghi un centesimo in più. La commissione non influenza i nostri consigli.',
    },
    ...
    'faq33.a': {
      de: 'Du kannst bis zu drei tägliche Erinnerungen einstellen (Morgens, Mittags, Abends). Eine Erinnerung wird <strong>nur dann</strong> ausgelöst, wenn mindestens eines deiner aktiven Gewohnheiten diesem Zeitslot zugeordnet ist – sonst bleibt sie still. Benachrichtigungen feuern in deiner Geräte-Zeitzone.',
    ...
    'faq40.a': {
      de: 'In der App findest du unter „Feedback" ein direktes Eingabeformular für Bugs, Feature-Wünsche oder Vorschläge für neue ARCs, Gewohnheiten und Lerninhalte. Jede Nachricht landet direkt im Inbox der Entwicklung – wir lesen alles.',
      en: 'In the app, under "Feedback", you’ll find a direct input form for bugs, feature requests or suggestions for new ARCs, habits and learning content. Every message lands directly in the development inbox — we read everything.',
      es: 'En la app, en «Feedback», encuentras un formulario directo para bugs, sugerencias de funciones o propuestas de nuevos ARCs, hábitos y contenidos de aprendizaje. Cada mensaje llega directamente al buzón de desarrollo — leemos todo.',
      fr: "Dans l'app, sous « Feedback », tu trouves un formulaire direct pour les bugs, demandes de fonctionnalités ou suggestions de nouveaux ARC, habitudes et contenus d'apprentissage. Chaque message arrive directement dans la boîte de réception du développement — on lit tout.",
      it: 'Nell’app, in «Feedback», trovi un modulo diretto per bug, richieste di funzionalità o proposte di nuovi ARC, abitudini e contenuti di apprendimento. Ogni messaggio arriva direttamente nella inbox di sviluppo — leggiamo tutto.',
    },
  };
