...
(function () {
  ...
  const DICT = {
    ...
    'arcs.sub':      { de: 'Drei reale ARCs aus der Sammlung — jeweils mit Dauer, Lebensbereich und den echten Gewohnheiten, Werttypen und Zielwerten, die du trackst. Der große baut sich in Stufen auf.', en: 'Three real ARCs from the library — each with duration, life area and the actual habits, value types and targets you track. The big one builds up in stages.', es: 'Tres ARCs reales de la biblioteca — cada uno con duración, área de vida y los hábitos, tipos de valor y objetivos reales que registras. El grande se construye por etapas.', fr: "Trois ARC réels de la bibliothèque — chacun avec durée, domaine de vie et les habitudes, types de valeur et objectifs réels que tu suis. Le grand se construit par niveaux.", it: 'Tre ARC reali dalla biblioteca — ognuno con durata, area di vita e le abitudini, i tipi di valore e gli obiettivi reali che tracci. Quello grande si costruisce per livelli.' },

    // ARC — the real Classic Winter ARC (Body, 8 weeks, Hard, STAGED build-up) — Supabase arc id 20
    'arc1.area':  { de: 'Körper', en: 'Body', es: 'Cuerpo', fr: 'Corps', it: 'Corpo' },
    'arc1.badge': { de: 'Stufenmodell', en: 'Build-up', es: 'Progresión', fr: 'Progression', it: 'Progressione' },
    ...
    'arc1.b':    { de: 'Acht Wochen Lock-in für die dunkle Jahreszeit — trainieren, erholen, lesen und Ablenkung rausschneiden. Du startest mit dem Kern, und jede starke Woche schaltet die nächste Stufe frei.', en: 'Eight weeks to lock in over the dark season — train, recover, read and cut the noise. You start with the core, and every strong week unlocks the next stage.', es: 'Ocho semanas de bloqueo en la estación oscura — entrenar, recuperar, leer y cortar el ruido. Empiezas con el núcleo, y cada semana fuerte desbloquea la siguiente etapa.', fr: "Huit semaines de lock-in pour la saison sombre — t'entraîner, récupérer, lire et couper le bruit. Tu commences par le noyau, et chaque semaine forte débloque le niveau suivant.", it: 'Otto settimane di lock-in nella stagione buia — allenarti, recuperare, leggere e tagliare il rumore. Parti dal nucleo, e ogni settimana forte sblocca il livello successivo.' },
    'arc1.s1':   { de: 'Stufe 1 · Start', en: 'Stage 1 · start', es: 'Etapa 1 · inicio', fr: 'Niveau 1 · début', it: 'Livello 1 · inizio' },
    'arc1.s2':   { de: 'Stufe 2 · nach 1 starken Woche', en: 'Stage 2 · after 1 strong week', es: 'Etapa 2 · tras 1 semana fuerte', fr: 'Niveau 2 · après 1 semaine forte', it: 'Livello 2 · dopo 1 settimana forte' },
    'arc1.s3':   { de: 'Stufe 3 · nach 2 starken Wochen', en: 'Stage 3 · after 2 strong weeks', es: 'Etapa 3 · tras 2 semanas fuertes', fr: 'Niveau 3 · après 2 semaines fortes', it: 'Livello 3 · dopo 2 settimane forti' },
    ...
    'arc1.h2':   { de: 'Krafttraining ≥ 120 min/Woche', en: 'Strength Training ≥ 120 min/week', es: 'Entrenamiento de fuerza ≥ 120 min/semana', fr: 'Renforcement musculaire ≥ 120 min/semaine', it: 'Allenamento di forza ≥ 120 min/settimana' },
    'arc1.h3':   { de: 'Früh aufstehen · 5×/Woche', en: 'Wake Up Early · 5×/week', es: 'Levantarse temprano · 5×/semana', fr: 'Se lever tôt · 5×/semaine', it: 'Svegliarsi presto · 5×/settimana' },
    'arc1.h4':   { de: 'Kein Snooze · täglich', en: 'No Snooze · daily', es: 'Sin posponer la alarma · a diario', fr: 'Pas de snooze · chaque jour', it: 'Niente snooze · ogni giorno' },
    'arc1.h5':   { de: 'Lesen 10 → 25 Seiten', en: 'Read 10 → 25 pages', es: 'Leer 10 → 25 páginas', fr: 'Lire 10 → 25 pages', it: 'Leggere 10 → 25 pagine' },
    'arc1.h6':   { de: 'Dehnen ≥ 10 min', en: 'Stretching ≥ 10 min', es: 'Estiramientos ≥ 10 min', fr: 'Étirements ≥ 10 min', it: 'Stretching ≥ 10 min' },
    'arc1.h7':   { de: 'Kältereiz · täglich', en: 'Cold Exposure · daily', es: 'Exposición al frío · a diario', fr: 'Exposition au froid · chaque jour', it: 'Esposizione al freddo · ogni giorno' },
    'arc1.h8':   { de: 'Social-Media-Zeit ≤ 1 h', en: 'Social Media Time ≤ 1 h', es: 'Tiempo en redes sociales ≤ 1 h', fr: 'Temps sur les réseaux sociaux ≤ 1 h', it: 'Tempo sui social ≤ 1 h' },
    'arc1.h9':   { de: 'Atemübungen ≥ 15 min', en: 'Breathwork ≥ 15 min', es: 'Ejercicios de respiración ≥ 15 min', fr: 'Exercices de respiration ≥ 15 min', it: 'Esercizi di respirazione ≥ 15 min' },
    'arc1.h10':  { de: 'Meditation ≥ 10 min', en: 'Meditation ≥ 10 min', es: 'Meditación ≥ 10 min', fr: 'Méditation ≥ 10 min', it: 'Meditazione ≥ 10 min' },
    'arc1.h11':  { de: 'Tagebuch schreiben · täglich', en: 'Journaling · daily', es: 'Escribir un diario · a diario', fr: 'Tenir un journal · chaque jour', it: 'Scrivere un diario · ogni giorno' },
    ...
    'sys.title':    { de: 'Fünf Tabs. Eine klare Logik.', en: 'Five tabs. One clear logic.', es: 'Cinco pestañas. Una lógica clara.', fr: 'Cinq onglets. Une logique claire.', it: 'Cinque tab. Una logica chiara.' },
    'sys.sub':      { de: 'Tracken, wählen, auswerten, lernen, reflektieren — jeder Tab macht genau eine Sache, dafür richtig.', en: 'Track, choose, analyze, learn, reflect — each tab does one thing, and does it well.', es: 'Registrar, elegir, analizar, aprender, reflexionar — cada pestaña hace una sola cosa, y bien.', fr: 'Suivre, choisir, analyser, apprendre, réfléchir — chaque onglet fait une seule chose, et bien.', it: 'Tracciare, scegliere, analizzare, imparare, riflettere — ogni tab fa una cosa sola, e la fa bene.' },

    // 01 Home
    'sc1.num':   { de: '01 — Home', en: '01 — Home', es: '01 — Inicio', fr: '01 — Accueil', it: '01 — Home' },
    'sc1.h':     { de: 'Dein Tag auf einen Blick.', en: 'Your day at a glance.', es: 'Tu día de un vistazo.', fr: "Ta journée d'un coup d'œil.", it: "La tua giornata a colpo d'occhio." },
    'sc1.b':     { de: 'Drei Ringe, ein Blick: Tag, Woche, Level. Darunter deine Gewohnheiten für heute — jede mit ihrem eigenen Maßstab, vom genauen Zielwert bis zum simplen Ja/Nein. Und deine aktiven ARCs hast du immer dabei.', en: "Three rings, one glance: day, week, level. Below them, today's habits — each with its own measure, from an exact target down to a plain yes/no. And your active ARCs are always right there.", es: 'Tres anillos, un vistazo: día, semana, nivel. Debajo, tus hábitos de hoy — cada uno con su propia medida, desde un objetivo exacto hasta un simple sí/no. Y tus ARCs activos, siempre a mano.', fr: "Trois anneaux, un coup d'œil : jour, semaine, niveau. En dessous, tes habitudes du jour — chacune avec sa propre mesure, d'une cible précise à un simple oui/non. Et tes ARC actifs, toujours sous la main.", it: 'Tre anelli, un colpo d’occhio: giorno, settimana, livello. Sotto, le tue abitudini di oggi — ognuna con la propria misura, da un obiettivo preciso a un semplice sì/no. E i tuoi ARC attivi sempre a portata.' },
    ...
    // 02 ARCademy — ARCs + Gewohnheiten
    'sc2.num': { de: '02 — ARCademy', en: '02 — ARCademy', es: '02 — ARCademy', fr: '02 — ARCademy', it: '02 — ARCademy' },
    'sc2.h':   { de: 'Deine nächste Challenge.', en: 'Your next challenge.', es: 'Tu próximo reto.', fr: 'Ton prochain défi.', it: 'La tua prossima sfida.' },
    'sc2.b':   { de: 'Die ARCademy hat zwei Tabs. Im ARC-Tab suchst du dir eine fertige Challenge von 1 bis 12 Wochen — filterbar nach Schwierigkeit, Lebensbereich und Dauer — oder du baust dir selbst eine, im Assistenten oder per KI. Der Gewohnheiten-Tab öffnet die Bausteine dahinter: jede einzelne Gewohnheit, sauber in Ober- und Unterkategorien sortiert.', en: 'ARCademy has two tabs. In the ARC tab you grab a ready-made challenge from 1 to 12 weeks — filter by difficulty, life area or duration — or build your own, in the wizard or with AI. The Habits tab opens up the building blocks: every single habit, neatly sorted into parent and child categories.', es: 'ARCademy tiene dos pestañas. En la de ARCs coges un reto listo de 1 a 12 semanas — filtras por dificultad, área o duración — o creas el tuyo, en el asistente o con IA. La pestaña de Hábitos abre las piezas que hay detrás: cada hábito, ordenado en categorías padre e hija.', fr: "ARCademy a deux onglets. Dans l'onglet ARC, tu attrapes un défi prêt à l'emploi de 1 à 12 semaines — filtre par difficulté, domaine ou durée — ou tu construis le tien, dans l'assistant ou avec l'IA. L'onglet Habitudes ouvre les briques derrière : chaque habitude, rangée en catégories parentes et enfants.", it: 'ARCademy ha due tab. Nel tab ARC prendi una sfida pronta da 1 a 12 settimane — filtri per difficoltà, area o durata — oppure la costruisci tu, nell’assistente o con l’IA. Il tab Abitudini apre i mattoni che stanno dietro: ogni singola abitudine, ordinata in categorie padre e figlie.' },
    'sc2.li1': { de: 'Fertige ARCs mit Theorie, Tipps, Titelbild und passendem Lerninhalt', en: 'Ready-made ARCs with theory, tips, cover and matching learning content', es: 'ARCs listos con teoría, consejos, portada y contenido de aprendizaje a juego', fr: "Des ARC prêts à l'emploi avec théorie, conseils, couverture et contenu d'apprentissage adapté", it: 'ARC pronti con teoria, consigli, copertina e contenuto di apprendimento adatto' },
    'sc2.li2': { de: 'Gewohnheiten-Tab: jede Gewohnheit messbar mit Ziel, Richtung und Einheit — „Zone 2 Laufen = 12 km" zählt automatisch auch für <em>Laufen</em> und <em>Ausdauertraining</em>', en: 'Habits tab: every habit measurable with target, direction and unit — “Zone 2 Run = 12 km” automatically counts toward <em>Running</em> and <em>Endurance training</em> too', es: 'Pestaña de Hábitos: cada hábito medible con objetivo, dirección y unidad — «Correr Zona 2 = 12 km» cuenta automáticamente también para <em>Correr</em> y <em>Entrenamiento de resistencia</em>', fr: "Onglet Habitudes : chaque habitude mesurable avec cible, direction et unité — « Course Zone 2 = 12 km » compte aussi automatiquement pour <em>Course</em> et <em>Endurance</em>", it: 'Tab Abitudini: ogni abitudine misurabile con obiettivo, direzione e unità — «Corsa Zona 2 = 12 km» conta in automatico anche per <em>Corsa</em> e <em>Allenamento di resistenza</em>' },
    'sc2.li3': { de: 'KI-Generator: Ziel in einem Satz beschreiben, fertigen ARC bekommen — editierbar', en: 'AI generator: describe your goal in one sentence, get a ready ARC — editable', es: 'Generador IA: describe tu meta en una frase, recibe un ARC listo — editable', fr: 'Générateur IA : décris ton objectif en une phrase, reçois un ARC prêt — éditable', it: 'Generatore IA: descrivi il tuo obiettivo in una frase, ricevi un ARC pronto — modificabile' },
    'sc2.li4': { de: 'Eigene ARCs und eigene Gewohnheiten Schritt für Schritt bauen — Einheit, Richtung und Intervall frei wählen <span class="pro-pill">Pro</span>', en: 'Build your own ARCs and your own habits step by step — pick unit, direction and interval <span class="pro-pill">Pro</span>', es: 'Crea tus propios ARCs y tus propios hábitos paso a paso — elige unidad, dirección e intervalo <span class="pro-pill">Pro</span>', fr: "Construis tes propres ARC et tes propres habitudes étape par étape — choisis l'unité, la direction et l'intervalle <span class=\"pro-pill\">Pro</span>", it: 'Costruisci i tuoi ARC e le tue abitudini passo dopo passo — scegli unità, direzione e intervallo <span class="pro-pill">Pro</span>' },
    'sc2.li5': { de: 'Ständig neue ARCs, Gewohnheiten und Lerninhalte — dazu saisonale ARCs zum Jahreswechsel oder Sommerstart', en: 'New ARCs, habits and learning content all the time — plus seasonal ARCs for New Year or the start of summer', es: 'Nuevos ARCs, hábitos y contenidos sin parar — más ARCs de temporada para Año Nuevo o el inicio del verano', fr: "De nouveaux ARC, habitudes et contenus en continu — plus des ARC saisonniers pour le Nouvel An ou le début de l'été", it: 'Sempre nuovi ARC, abitudini e contenuti — più ARC stagionali per Capodanno o l’inizio dell’estate' },
    'sc2.alt': { de: 'ARCademy — ARC-Tab mit fertigen Challenges, Suche und Filter', en: 'ARCademy — ARC tab with ready-made challenges, search and filter', es: 'ARCademy — pestaña de ARCs con retos listos, búsqueda y filtros', fr: 'ARCademy — onglet ARC avec des défis prêts à l’emploi, recherche et filtres', it: 'ARCademy — tab ARC con sfide pronte, ricerca e filtri' },
    'sc2.alt2': { de: 'ARCademy — Gewohnheiten-Tab mit Ober- und Unterkategorien', en: 'ARCademy — Habits tab with categories and subcategories', es: 'ARCademy — pestaña de Hábitos con categorías y subcategorías', fr: 'ARCademy — onglet Habitudes avec catégories et sous-catégories', it: 'ARCademy — tab Abitudini con categorie e sottocategorie' },

    // 03 Insights
    'sc3.num': { de: '03 — Insights', en: '03 — Insights', es: '03 — Insights', fr: '03 — Insights', it: '03 — Insights' },
    'sc3.h':   { de: 'Schwarz auf weiß, was wirklich läuft.', en: "In black and white: what's actually working.", es: 'Negro sobre blanco: qué funciona de verdad.', fr: 'Noir sur blanc : ce qui marche vraiment.', it: 'Nero su bianco: cosa funziona davvero.' },
    'sc3.b':   { de: 'Drei Tabs voller Auswertungen, alle aus deinen echten Logs — nichts geschönt. Overall gibt dir dein 90-Tage-Profil, der ARC-Tab zeigt Erfolgsquote und Verteilung deiner Challenges, und im Gewohnheiten-Tab nimmst du jede einzelne Gewohnheit unter die Lupe.', en: 'Three tabs full of analysis, all from your real logs — nothing sugar-coated. Overall gives you your 90-day profile, the ARC tab shows the success rate and spread of your challenges, and the Habits tab puts every single habit under the microscope.', es: 'Tres pestañas llenas de análisis, todo desde tus registros reales — sin maquillar. Overall te da tu perfil de 90 días, la pestaña de ARCs muestra la tasa de éxito y el reparto de tus retos, y la de Hábitos pone cada hábito bajo la lupa.', fr: "Trois onglets pleins d'analyses, le tout depuis tes vrais logs — rien d'enjolivé. Overall te donne ton profil sur 90 jours, l'onglet ARC montre le taux de réussite et la répartition de tes défis, et l'onglet Habitudes passe chaque habitude au microscope.", it: 'Tre tab pieni di analisi, tutto dai tuoi log reali — niente abbellito. Overall ti dà il profilo di 90 giorni, il tab ARC mostra tasso di successo e distribuzione delle tue sfide, e il tab Abitudini mette ogni abitudine sotto la lente.' },
    'sc3.li1': { de: 'Overall-Profil über 90 Tage plus ARC-Auswertung: Erfolgsquote, Dauer und Verteilung über deine Lebensbereiche', en: '90-day overall profile plus ARC analysis: success rate, duration and spread across your life areas', es: 'Perfil general de 90 días más análisis de ARCs: tasa de éxito, duración y reparto por tus áreas de vida', fr: 'Profil global sur 90 jours plus analyse des ARC : taux de réussite, durée et répartition sur tes domaines de vie', it: 'Profilo generale su 90 giorni più analisi degli ARC: tasso di successo, durata e distribuzione sulle tue aree di vita' },
    'sc3.li2': { de: 'Jede Gewohnheit einzeln unter der Lupe: Heatmap, Wert-Verlauf und Streaks — umschaltbar von Tag bis Gesamt <span class="pro-pill">Pro</span>', en: 'Every habit under the microscope: heatmap, value trend and streaks — switch from day to all-time <span class="pro-pill">Pro</span>', es: 'Cada hábito bajo la lupa: heatmap, evolución del valor y rachas — de día a histórico <span class="pro-pill">Pro</span>', fr: 'Chaque habitude au microscope : heatmap, évolution de la valeur et séries — du jour au total <span class="pro-pill">Pro</span>', it: 'Ogni abitudine sotto la lente: heatmap, andamento del valore e streak — da giorno a totale <span class="pro-pill">Pro</span>' },
    'sc3.li3': { de: 'Unter-Gewohnheiten rollen automatisch hoch — „Zone 2 Laufen" summiert sich unter <em>Laufen</em> und <em>Ausdauertraining</em>, dazu Korrelationen zwischen deinen Gewohnheiten <span class="pro-pill">Pro</span>', en: 'Sub-habits roll up automatically — “Zone 2 Run” adds up under <em>Running</em> and <em>Endurance training</em>, plus correlations between your habits <span class="pro-pill">Pro</span>', es: 'Los subhábitos se agregan solos — «Correr Zona 2» suma bajo <em>Correr</em> y <em>Entrenamiento de resistencia</em>, más correlaciones entre tus hábitos <span class="pro-pill">Pro</span>', fr: "Les sous-habitudes s'agrègent seules — « Course Zone 2 » s'additionne sous <em>Course</em> et <em>Endurance</em>, plus les corrélations entre tes habitudes <span class=\"pro-pill\">Pro</span>", it: 'Le sotto-abitudini si sommano da sole — «Corsa Zona 2» rientra sotto <em>Corsa</em> e <em>Allenamento di resistenza</em>, più le correlazioni tra le tue abitudini <span class="pro-pill">Pro</span>' },
    'sc3.li4': { de: 'Jede Zahl kommt aus deinen echten Logs — privat, nichts wird geteilt', en: 'Every number comes from your real logs — private, nothing gets shared', es: 'Cada número sale de tus registros reales — privado, nada se comparte', fr: "Chaque chiffre vient de tes vrais logs — privé, rien n'est partagé", it: 'Ogni numero viene dai tuoi log reali — privato, niente viene condiviso' },
    'sc3.alt': { de: 'Insights — Auswertungen aus deinen echten Logs', en: 'Insights — analysis from your real logs', es: 'Insights — análisis desde tus registros reales', fr: 'Insights — analyses depuis tes vrais logs', it: 'Insights — analisi dai tuoi log reali' },

    // 04 Discover · Lerninhalte
    'sc4.num': { de: '04 — Discover', en: '04 — Discover', es: '04 — Discover', fr: '04 — Discover', it: '04 — Discover' },
    'sc4.h':   { de: 'Versteh deine Gewohnheiten.', en: 'Understand your habits.', es: 'Entiende tus hábitos.', fr: 'Comprends tes habitudes.', it: 'Capisci le tue abitudini.' },
    'sc4.b':   { de: 'Zu jeder Gewohnheit und jedem ARC gehört ein eigener Lerninhalt — von uns geschrieben, in vier bis acht Minuten gelesen und genau dort verlinkt, wo du ihn brauchst. Kein Nacherzählen dicker Bücher, nur die Kernidee auf den Punkt.', en: 'Every habit and every ARC comes with its own lesson — written by us, read in four to eight minutes and linked right where you need it. No recap of thick books, just the core idea, to the point.', es: 'Cada hábito y cada ARC trae su propio contenido de aprendizaje — escrito por nosotros, leído en cuatro a ocho minutos y enlazado justo donde lo necesitas. Nada de resumir libros enteros, solo la idea central, al grano.', fr: "Chaque habitude et chaque ARC vient avec son propre contenu d'apprentissage — écrit par nous, lu en quatre à huit minutes et lié là où il faut. Pas de résumé de gros livres, juste l'idée centrale, droit au but.", it: 'Ogni abitudine e ogni ARC ha il suo contenuto di apprendimento — scritto da noi, letto in quattro-otto minuti e collegato proprio dove serve. Niente riassunti di libroni, solo l’idea centrale, dritti al punto.' },
    'sc4.li1': { de: 'Eigene Lerninhalte zu erprobten Themen: Trainingswissenschaft, Schlaf, Atemtechnik, Lernen und Fokus', en: 'Our own lessons on proven topics: training science, sleep, breathing, learning and focus', es: 'Contenidos propios sobre temas probados: ciencia del entrenamiento, sueño, respiración, aprendizaje y foco', fr: "Nos propres contenus sur des sujets éprouvés : science de l'entraînement, sommeil, respiration, apprentissage et concentration", it: "Contenuti propri su temi collaudati: scienza dell'allenamento, sonno, respirazione, apprendimento e focus" },
    'sc4.li2': { de: 'Eine klare These pro Lerninhalt, sofort anwendbar — ohne geschützte Inhalte nachzudrucken', en: 'One clear thesis per lesson, ready to apply — without reprinting protected content', es: 'Una tesis clara por contenido, lista para aplicar — sin reproducir contenido protegido', fr: 'Une thèse claire par contenu, applicable tout de suite — sans reproduire de contenu protégé', it: 'Una tesi chiara per contenuto, subito applicabile — senza riprodurre contenuti protetti' },
    'sc4.li3': { de: 'Optionale Buch- und Talk-Tipps, mit * als Affiliate-Links markiert — ohne Mehrkosten für dich', en: 'Optional book and talk picks, marked with * as affiliate links — at no extra cost to you', es: 'Recomendaciones opcionales de libros y charlas, marcadas con * como enlaces de afiliado — sin coste extra para ti', fr: "Suggestions optionnelles de livres et conférences, marquées d'un * comme liens affiliés — sans surcoût pour toi", it: 'Consigli opzionali di libri e talk, marcati con * come link affiliati — senza costi extra per te' },
    'sc4.alt': { de: 'Discover — Lerninhalte mit eigenen redaktionellen ArcUp-Texten', en: 'Discover — Learning Content written in-house by ArcUp', es: 'Discover — Contenidos de aprendizaje propios de ArcUp', fr: "Discover — Contenus d'apprentissage rédigés par ArcUp", it: 'Discover — Contenuti di apprendimento scritti da ArcUp' },

    // 05 Compass
    'sc5.num': { de: '05 — Compass', en: '05 — Compass', es: '05 — Compass', fr: '05 — Compass', it: '05 — Compass' },
    'sc5.h':   { de: 'Das große Ganze.', en: 'The big picture.', es: 'El panorama completo.', fr: "La vue d'ensemble.", it: 'Il quadro completo.' },
    'sc5.b':   { de: 'Bewerte deine sechs Lebensbereiche von 1 bis 10 und sieh im Lebensrad sofort, wo die Balance kippt. Zu jedem Bereich schreibst du deine Vision, setzt Ziele und hältst im Tagebuch fest, was zählt — durchsuchbar und jederzeit editierbar.', en: 'Rate your six life areas from 1 to 10 and let the Wheel of Life show you at a glance where balance tips. For each area you write your vision, set goals and capture what matters in the journal — searchable and editable any time.', es: 'Puntúa tus seis áreas de vida de 1 a 10 y deja que la Rueda de la vida te muestre de un vistazo dónde se rompe el equilibrio. En cada área escribes tu visión, fijas metas y anotas lo que importa en el diario — buscable y editable cuando quieras.', fr: "Note tes six domaines de vie de 1 à 10 et laisse la Roue de la vie te montrer d'un coup d'œil où l'équilibre bascule. Pour chaque domaine, tu écris ta vision, fixes des objectifs et notes ce qui compte dans le journal — cherchable et éditable à tout moment.", it: 'Valuta le tue sei aree di vita da 1 a 10 e lascia che la Ruota della vita ti mostri a colpo d’occhio dove l’equilibrio salta. Per ogni area scrivi la tua visione, fissi obiettivi e annoti ciò che conta nel diario — ricercabile e modificabile quando vuoi.' },
    'sc5.li1': { de: 'Interaktives Lebensrad über Körper, Geist, Karriere, Beziehungen, Lebensstil und Ernährung', en: 'Interactive Wheel of Life across Body, Mind, Career, Relationships, Lifestyle and Nutrition', es: 'Rueda de la vida interactiva sobre Cuerpo, Mente, Carrera, Relaciones, Estilo de vida y Nutrición', fr: 'Roue de la vie interactive sur Corps, Esprit, Carrière, Relations, Style de vie et Nutrition', it: 'Ruota della vita interattiva su Corpo, Mente, Carriera, Relazioni, Stile di vita e Alimentazione' },
    'sc5.li2': { de: 'Vision-Statement pro Bereich <span class="pro-pill">Pro</span>', en: 'Vision statement per area <span class="pro-pill">Pro</span>', es: 'Declaración de visión por área <span class="pro-pill">Pro</span>', fr: "Énoncé de vision par domaine <span class=\"pro-pill\">Pro</span>", it: 'Dichiarazione di visione per area <span class="pro-pill">Pro</span>' },
    'sc5.liGoals': { de: 'Ziele mit Fälligkeit pro Bereich — wöchentlich, monatlich, jährlich <span class="pro-pill">Pro</span>', en: 'Goals with due dates per area — weekly, monthly, yearly <span class="pro-pill">Pro</span>', es: 'Objetivos con fecha por área — semanal, mensual, anual <span class="pro-pill">Pro</span>', fr: "Objectifs avec échéance par domaine — hebdo, mensuel, annuel <span class=\"pro-pill\">Pro</span>", it: 'Obiettivi con scadenza per area — settimanale, mensile, annuale <span class="pro-pill">Pro</span>' },
    'sc5.li3': { de: 'Tagebuch in vier Typen: Gedanke, Lernen, Dankbarkeit, Erfolge &amp; Verluste <span class="pro-pill">Pro</span>', en: 'Journal in four types: Thought, Learning, Gratitude, Wins &amp; Losses <span class="pro-pill">Pro</span>', es: 'Diario en cuatro tipos: Pensamiento, Aprendizaje, Gratitud, Logros y pérdidas <span class="pro-pill">Pro</span>', fr: "Journal en quatre types : Pensée, Apprentissage, Gratitude, Victoires et pertes <span class=\"pro-pill\">Pro</span>", it: 'Diario in quattro tipi: Pensiero, Apprendimento, Gratitudine, Vittorie e perdite <span class="pro-pill">Pro</span>' },
    'sc5.alt': { de: 'Compass — Lebensrad-Radar über deine Lebensbereiche', en: 'Compass — Wheel-of-Life radar across your life areas', es: 'Compass — radar Rueda de la vida sobre tus áreas de vida', fr: "Compass — radar Roue de la vie sur tes domaines de vie", it: 'Compass — radar Ruota della vita sulle tue aree di vita' },
    ...
    // ─── LEGAL PAGES ─────────────────────────────────────
    'legal.lastUpdated': {
      de: 'Stand: 13. Juli 2026',
      en: 'Last updated: 13 July 2026',
      es: 'Última actualización: 13 de julio de 2026',
      fr: 'Dernière mise à jour : 13 juillet 2026',
      it: 'Ultimo aggiornamento: 13 luglio 2026',
    },
