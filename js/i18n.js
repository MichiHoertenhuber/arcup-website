...
(function () {
  ...
  const DICT = {
    ...
    'mani.title': {
      de: 'Vier Prinzipien,<br />die aus Vorsätzen ein <em>System</em> machen.',
      en: 'Four principles<br />that turn resolutions into a <em>system</em>.',
      es: 'Cuatro principios<br />que convierten propósitos en un <em>sistema</em>.',
      fr: 'Quatre principes<br />qui transforment les résolutions en <em>système</em>.',
      it: 'Quattro principi<br />che trasformano i buoni propositi in un <em>sistema</em>.',
    },
    ...
    'sys.sub':      { de: 'Tracken, wählen, lernen, reflektieren — jeder Tab erfüllt genau eine Aufgabe. Keine überladenen Screens, keine Multifunktions-Buttons.', en: 'Track, choose, learn, reflect — each tab does exactly one job. No overloaded screens, no multi-function buttons.', es: 'Registrar, elegir, aprender, reflexionar — cada pestaña hace una sola cosa. Pantallas claras, sin botones multifunción.', fr: 'Suivre, choisir, apprendre, réfléchir — chaque onglet a une seule tâche. Pas d’écrans surchargés, pas de boutons multifonctions.', it: 'Tracciare, scegliere, imparare, riflettere — ogni tab fa una sola cosa. Schermate pulite, niente pulsanti multifunzione.' },

    // 01 Home
    'sc1.num':   { de: '01 — Home', en: '01 — Home', es: '01 — Inicio', fr: '01 — Accueil', it: '01 — Home' },
    'sc1.h':     { de: 'Heute. In Zahlen. Ehrlich.', en: 'Today. In numbers. Honest.', es: 'Hoy. En números. Honesto.', fr: "Aujourd'hui. En chiffres. Honnête.", it: 'Oggi. In numeri. Onesto.' },
    ...
    'sc1.li3':   { de: 'Logbuch öffnet sich in einem Tap — heute und gestern beschreibbar, nicht weiter zurück', en: 'The logbook opens in one tap — today and yesterday are writable, nothing earlier', es: 'El registro se abre en un toque — hoy y ayer son editables, nada anterior', fr: "Le journal s'ouvre en un tap — aujourd'hui et hier modifiables, rien avant", it: "Il logbook si apre con un tap — oggi e ieri scrivibili, niente prima" },
    ...
    'sc3b.b':   { de: 'Der zweite Discover-Tab: über 120 ausgewählte Gewohnheiten, sauber in Ober- und Unterkategorien strukturiert. Du trackst „Long Run = 12 km" — die Insights summieren das automatisch unter <em>Laufen</em> und <em>Ausdauertraining</em> mit.', en: 'The second Discover tab: over 120 selected habits, neatly structured into parent and child categories. You track "Long Run = 12 km" — your insights automatically roll it up under <em>Running</em> and <em>Endurance training</em>.', es: 'La segunda pestaña de Discover: más de 120 hábitos seleccionados, organizados con claridad en categorías padre e hija. Registras «Long Run = 12 km» — los insights lo agregan automáticamente bajo <em>Correr</em> y <em>Entrenamiento de resistencia</em>.', fr: "Le second onglet Discover : plus de 120 habitudes sélectionnées, organisées proprement en catégories parentes et enfants. Tu suis « Long Run = 12 km » — les insights l'agrègent automatiquement sous <em>Course</em> et <em>Endurance</em>.", it: 'Il secondo tab di Discover: oltre 120 abitudini selezionate, organizzate con chiarezza in categorie padre e figlie. Tracci «Long Run = 12 km» — gli insights lo aggregano automaticamente sotto <em>Corsa</em> e <em>Allenamento di resistenza</em>.' },
    'sc3b.li1': { de: 'Über 120 ausgewählte Gewohnheiten in einer Baum-Struktur — durchsuchbar und filterbar', en: 'Over 120 selected habits in a tree structure — searchable and filterable', es: 'Más de 120 hábitos seleccionados en estructura de árbol — buscables y filtrables', fr: "Plus de 120 habitudes sélectionnées dans une arborescence — recherchables et filtrables", it: 'Oltre 120 abitudini selezionate in una struttura ad albero — ricercabili e filtrabili' },
    'sc3b.li2': { de: 'Untergewohnheiten zählen automatisch mit: einmal tracken, mehrfach gezählt — von Long Run bis Ausdauertraining', en: 'Sub-habits count up automatically: track once, count multiple times — from Long Run up to Endurance training', es: 'Los sub-hábitos se suman automáticamente: registras una vez, cuenta varias veces — de Long Run a Entrenamiento de resistencia', fr: 'Les sous-habitudes se cumulent automatiquement : suivre une fois, compter plusieurs fois — du Long Run à l’endurance', it: 'I sotto-habit si sommano automaticamente: tracci una volta, conta più volte — dal Long Run fino all’allenamento di resistenza' },
    ...
    'faq22.a': {
      de: 'Der <strong>Free-Tarif</strong> umfasst:<ul><li>Tracking aller Gewohnheiten aus der ausgewählten Sammlung</li><li>Einen aktiven ARC gleichzeitig</li><li>Lebensbereiche mit Wheel of Life und Vision-Texten</li><li>Zugriff auf die Lerninhalte</li><li>Selbstbewertung (regelmäßige Updates pro Lebensbereich)</li></ul>',
      ...
      es: 'El plan <strong>Free</strong> incluye:<ul><li>Tracking de todos los hábitos de la biblioteca seleccionada</li><li>Un ARC activo a la vez</li><li>Áreas de vida con Wheel of Life y textos de visión</li><li>Acceso a los contenidos de aprendizaje</li><li>Autoevaluaciones (actualizar puntuación por área)</li></ul>',
      ...
      it: 'Il piano <strong>Free</strong> include:<ul><li>Tracciamento di tutte le abitudini della biblioteca selezionata</li><li>Un ARC attivo alla volta</li><li>Aree di vita con Wheel of Life e testi di visione</li><li>Accesso ai contenuti di apprendimento</li><li>Autovalutazioni (aggiornamento punteggi per area di vita)</li></ul>',
    },
    ...
    'faq38.a': {
      de: 'Nein. Inhalte vom Typ „ArcUp Original" werden <strong>vorab und redaktionell</strong> mit KI-Unterstützung erstellt – ohne Bezug zu individuellen Nutzerprofilen. Wenn du den AI ARC Generator nutzt, geht <strong>ausschließlich dein eingetippter Prompt</strong> an Anthropic – keine App-Daten, keine personenbezogenen Informationen.',
      ...
      es: 'No. Los contenidos «ArcUp Original» se crean <strong>de antemano y de forma editorial</strong> con apoyo de IA — sin relación con perfiles de usuario individuales. Cuando usas el AI ARC Generator, <strong>solo tu prompt</strong> se envía a Anthropic — sin datos de la app, sin información personal.',
      ...
      it: 'No. I contenuti «ArcUp Original» sono creati <strong>in anticipo e in modo editoriale</strong> con supporto IA — indipendentemente dai profili utente. Quando usi l’AI ARC Generator, <strong>solo il tuo prompt</strong> viene inviato ad Anthropic — nessun dato dell’app, nessuna informazione personale.',
    },
    'faq41.q': { de: 'Wie hänge ich einen Screenshot an mein Feedback an?', en: 'How do I attach a screenshot to my feedback?', es: '¿Cómo adjunto una captura de pantalla a mi feedback?', fr: 'Comment joindre une capture d’écran à mon retour ?', it: 'Come allego uno screenshot al mio feedback?' },
    'faq41.a': {
      de: 'Im Feedback-Formular tippst du unter „Screenshot (optional)" auf „Screenshot hinzufügen" und wählst ein Bild aus deiner Galerie – besonders praktisch bei Bugs, weil ein Bild das Problem oft schneller zeigt als Worte. Vor dem Senden kannst du es über das ×-Symbol wieder entfernen. Der Screenshot wird <strong>privat</strong> gespeichert und ist ausschließlich für das Entwicklungsteam einsehbar. Er ist Teil deines Datenexports und wird bei einer Konto-Löschung mitgelöscht.',
      en: 'In the feedback form, tap "Add screenshot" under "Screenshot (optional)" and pick an image from your gallery — especially handy for bugs, since a picture often shows the problem faster than words. You can remove it again via the × button before sending. The screenshot is stored <strong>privately</strong> and is visible only to the development team. It is part of your data export and is deleted together with your account.',
      es: 'En el formulario de feedback, toca «Añadir captura» bajo «Captura de pantalla (opcional)» y elige una imagen de tu galería — especialmente útil para errores, ya que una imagen suele mostrar el problema más rápido que las palabras. Puedes quitarla con el botón × antes de enviar. La captura se guarda de forma <strong>privada</strong> y solo es visible para el equipo de desarrollo. Forma parte de tu exportación de datos y se elimina junto con tu cuenta.',
      fr: "Dans le formulaire de retour, appuie sur « Ajouter une capture » sous « Capture d’écran (facultatif) » et choisis une image dans ta galerie — particulièrement utile pour les bugs, car une image montre souvent le problème plus vite que les mots. Tu peux la retirer via le bouton × avant l’envoi. La capture est stockée <strong>de manière privée</strong> et n’est visible que par l’équipe de développement. Elle fait partie de ton export de données et est supprimée avec ton compte.",
      it: 'Nel modulo di feedback, tocca «Aggiungi screenshot» sotto «Screenshot (opzionale)» e scegli un’immagine dalla galleria — particolarmente utile per i bug, perché un’immagine mostra spesso il problema più velocemente delle parole. Puoi rimuoverla con il pulsante × prima dell’invio. Lo screenshot è archiviato in modo <strong>privato</strong> ed è visibile solo al team di sviluppo. Fa parte della tua esportazione dei dati e viene eliminato insieme al tuo account.',
    },
